...
from how_about_this_day.forms import AnswerForm, PlanCreateForm, UserCreateForm, UserLoginForm, CommentForm
...
def _list(): 
    ...
    return jsonify({"study_plan_list" : study_plan_list}) # 클라에 약속 목록 전송
...
def detail(plan_id):
    ...
    if request.methods == 'POST': # 댓글 작성 후 저장 요청
        ...
        if g.user == study_plan.user: # 사용자가 작성한 글이라면 -> True
            return jsonify({"study_plan" : study_plan, "plan_writer" : "True", "myComments" : myComments}) # GET요청: 클라에 약속 데이터와 답변 데이터 전송
        else: # 사용자가 작성한 글이 아니면
            return jsonify({"study_plan" : study_plan, "plan_writer" : 'False', "myComments" : myComments}) # GET요청: 클라에 약속 데이터와 답변 데이터 전송

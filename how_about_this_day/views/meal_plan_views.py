...
from how_about_this_day.models import MealPlan, MealPlanComment, User
import functools
...
def _list(): 
    ...
    return jsonify({"meal_plan_list" : meal_plan_list}) # 클라에 약속 목록 전송
...
def detail(plan_id):
    ...
    if request.method == 'POST': # 댓글 작성 후 저장 요청
        ...
        if g.user == meal_plan.user: # 사용자가 작성한 글이라면 -> True
            return jsonify({"meal_plan" : meal_plan, "plan_writer" : "True", "myComments" : myComments}) # GET요청: 클라에 약속 데이터와 답변 데이터 전송
        else: # 사용자가 작성한 글이 아니면
            return jsonify({"meal_plan" : meal_plan, "plan_writer" : 'False', "myComments" : myComments}) # GET요청: 클라에 약속 데이터와 답변 데이터 전송


@bp.route('/coment_delete/<int:comment_id>') # 댓글 삭제
def delete_comment(comment_id):
    comment = MealPlanComment.query.get_or_404(comment_id)

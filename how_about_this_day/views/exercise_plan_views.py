...
from how_about_this_day.models import ExercisePlan, ExercisePlanComment
...
def _list(): 
    ...
    tempDict = []
    for plan in exercise_plan_list.items:
    ...
    return jsonify({"exercise_plan_list" : tempDict})


@bp.route('/detail/<int:plan_id>/', methods=('GET', 'POST')) # 약속 상세
def detail(plan_id):
    form = CommentForm()
    exercise_plan = ExercisePlan.query.get(plan_id) # plan_id에 해당하는 약속을 plan에 저장
    ...
    if request.method == 'POST': # 댓글 작성 후 저장 요청
        ...
        if exercise_plan.modify_date:
            return jsonify({"id" : exercise_plan.id, 
                            "user" : exercise_plan.user.username,
                            "subject" : exercise_plan.subject, 
                            "content" : exercise_plan.content,
                            "create_date" : exercise_plan.create_date.strftime('%Y년 %m월 %d일 %H:%M'),
                            "modify_date" : exercise_plan.modify_date.strftime('%Y년 %m월 %d일 %H:%M'),
...
def delete_comment(comment_id):
    comment = ExercisePlanComment.query.get(comment_id)
...
def modify(plan_id): # 약속 수정 함수 -> 로그인이 필요한 기능 + 글 작성자가 본인이어야 함
    plan = ExercisePlan.query.get(plan_id)
